...
<!--  </mt-header>-->
  ...
  <div class="bigbox">
    ...
    <ul class="tabsbox">
      ...
      <li>
        <a @click="cur=1" :class="{active:cur==1}">已通过</a>
      </li>
      <li>
  ...
  <div class="tab-contents">
    <div v-show="cur==0">
      <ul class="lists">
        <li class="list" v-for="item in data" :key="item">
          <div style="width: 33.3%;float: left;text-align: center;" @click="purchase">
            <div v-for="i in item.name" :key="i">{{i}}</div>
          </div>
          <div style="width: 33.3%;float: left;text-align: center;position: relative;">
            <div
              style="text-align: center;position: absolute;top: 50%;left: 25%;-webkit-transform: translateY(-50%);-ms-transform: translateY(-50%);-o-transform: translateY(-50%);transform: translateY(-50%);">
              <div>{{item.Price}}</div>
              <div>{{item.data}}</div>
            </div>
          </div>
          <div style="width: 33.3%;float: left;text-align: center;color: red;position: relative;">
            <div
              style="text-align: center;position: absolute;top: 50%;left: 25%;-webkit-transform: translateY(-50%);-ms-transform: translateY(-50%);-o-transform: translateY(-50%);transform: translateY(-50%);">
              {{item.money}}
            </div>
    ...
    <div v-show="cur==1">
<!--      <ul class="lists">-->
<!--        <li class="list" v-for="item in data" :key="item"><div style="width: 33.3%;float: left;text-align: center;" @click="again">-->
<!--          <div  v-for="i in item.name" :key="i">{{i}}</div>-->
<!--        </div>-->
<!--          <div style="width: 33.3%;float: left;text-align: center;position: relative;">-->
...
<script>
    export default {
        ...
        methods: {
            ...
            getData () {
                let vm = this
                vm.$ajax.post('/api/lists', {}).then(function (data) {
                    vm.data = data.start
                })
            },
            purchase: function () {
